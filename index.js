...
// --- ចំណុចសំខាន់៖ ប្តូរយកមកពី Firebase Console របស់អ្នក ---
...
app.post("/api/index", async (req, res) => {
  try {
    ...
    // ត្រូវជាមួយ Firebase: pendingAmount "$9.99"
    if (messageText.includes("9.99")) {
      const paymentQuery = query(
        collection(db, "payments"), // ប្តូរពី users មក payments វិញតាមរូបភាពរបស់អ្នក
        where("status", "==", "pending"),
        where("amount", "==", 9.99) // ប្តូរពី pendingAmount មក amount និងប្រើលេខ 9.99 (គ្មាន "")
      );
